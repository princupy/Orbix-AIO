...
async function execute({ message, args }) {
  ...
  if (!userId) {
    await message.reply(cv2Payload(vcContainer({ type: 'error', title: 'Invalid Usage', description: '**Usage:** `LR!vcdeafen @user`' })));
    return;
  }
  ...
  if (member.voice.serverDeaf) {
    await message.reply(cv2Payload(vcContainer({ type: 'warning', title: 'Already Deafened', description: `${member} is already server deafened.` })));
    return;
  }

  try {
    await member.voice.setDeaf(true, `vcdeafen by ${message.author.tag}`);
    ...
    await message.reply(cv2Payload(vcContainer({ type: 'error', title: 'Deafen Failed', description: err.code === 50013 ? 'Missing role hierarchy or **Deafen Members** permission.' : err.message })));
  }
}
...
module.exports = {
  ...
  aliases: ['voicedeafen'],
  category: 'voice',
  description: 'Server deafens a user in their voice channel.',
  usage: 'LR!vcdeafen @user',
  execute,

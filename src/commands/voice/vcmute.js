...
async function execute({ message, args }) {
  ...
  if (!userId) {
    await message.reply(cv2Payload(vcContainer({ type: 'error', title: 'Invalid Usage', description: '**Usage:** `LR!vcmute @user`' })));
    return;
  }
  ...
  if (member.voice.serverMute) {
    await message.reply(cv2Payload(vcContainer({ type: 'warning', title: 'Already Muted', description: `${member} is already server muted.` })));
    return;
  }

  try {
    await member.voice.setMute(true, `vcmute by ${message.author.tag}`);
    ...
    await message.reply(cv2Payload(vcContainer({ type: 'error', title: 'Mute Failed', description: err.code === 50013 ? 'Missing role hierarchy or **Mute Members** permission.' : err.message })));
  }
}
...
module.exports = {
  ...
  aliases: ['voicemute'],
  category: 'voice',
  description: 'Server mutes a user in their voice channel.',
  usage: 'LR!vcmute @user',
  execute,

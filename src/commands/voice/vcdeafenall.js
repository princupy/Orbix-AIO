...
async function execute({ message }) {
  ...
  if (!members.length) {
    await message.reply(cv2Payload(vcContainer({ type: 'warning', title: 'All Already Deafened', description: `All members in ${authorVc} are already server deafened.` })));
    return;
  }
  ...
  for (const m of members) {
    try {
      await m.voice.setDeaf(true, `vcdeafenall by ${message.author.tag}`);
      done++;
    } catch {
      failed++;
    }
  ...
  await message.channel.send(cv2Payload(vcContainer({
    ...
    title: done > 0 ? 'Members Deafened' : 'Deafen Failed',
    description: [
      `> **Channel:** ${authorVc}`,
      `> **Deafened:** ${done} member${done === 1 ? '' : 's'}`,
...
module.exports = {
  name: 'vcdeafenall',
  aliases: ['voicedeafenall'],
  category: 'voice',
  description: 'Server deafens all members in your current voice channel.',
  usage: 'LR!vcdeafenall',
  execute,
